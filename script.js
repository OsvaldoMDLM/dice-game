...
const getDices = () => {
	...
	dicesContainerTarget.innerHTML = `<img src='./img/dices/cd${dice1}.png' alt='Dado 1' /> <img src='./img/dices/cd${dice2}.png' alt='Dado' width="100%" />`
	return { dice1, dice2 }
}
...
btnFiredTarget.addEventListener('click', () => {
	...
	btnFiredTarget.style.display = 'none'
	dicesContainerTarget.innerHTML = ''
	resultsTarget.innerHTML = `<p>Tuviste una ganancia de: <strong class=${profit > 0 ? "won" : "lose"}>${profit}</strong><p>Tu saldo final es: ${document.getElementById('available-to-bet').value}</p>`
